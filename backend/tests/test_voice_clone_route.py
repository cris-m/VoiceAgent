...
@pytest.mark.asyncio
async def test_clone_rejects_non_pocket_tts(
...
@pytest.mark.asyncio
async def test_clone_missing_name_returns_422(
    async_client: AsyncClient, mock_voice_pipeline, mock_agent_client
):
    with patch(_PATCH_PIPELINE, return_value=mock_voice_pipeline), \
         patch(_PATCH_AGENT, return_value=mock_agent_client), \
...
@pytest.mark.asyncio
async def test_clone_no_file_returns_422(
    async_client: AsyncClient, mock_voice_pipeline, mock_agent_client
):
    with patch(_PATCH_PIPELINE, return_value=mock_voice_pipeline), \
         patch(_PATCH_AGENT, return_value=mock_agent_client), \
...
@pytest.mark.asyncio
async def test_clone_happy_path_returns_voice_id(
    async_client: AsyncClient, mock_voice_pipeline, mock_agent_client
):
...
@pytest.mark.asyncio
async def test_list_clones_happy_path(
    async_client: AsyncClient, mock_voice_pipeline, mock_agent_client
):
    from services.tts.base import Voice

    cloned_voice = Voice(
        id="clone_ab12cd34", name="My Clone", language="en",
        gender=None, description=None, metadata={"is_cloned": True}
    )
...
@pytest.mark.asyncio
async def test_list_clones_empty_list(
    async_client: AsyncClient, mock_voice_pipeline, mock_agent_client
):
...
@pytest.mark.asyncio
async def test_delete_clone_not_found_returns_404(
    async_client: AsyncClient, mock_voice_pipeline, mock_agent_client
):
...
@pytest.mark.asyncio
async def test_delete_clone_happy_path(
    async_client: AsyncClient, mock_voice_pipeline, mock_agent_client
):
